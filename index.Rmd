...
Finally the trance playlist seems to contain slightly more tracks in a minor key. I had expected this, due to the fact that minor keys are a common occurence in trance music.
...
```{r}
depeche<-
  get_tidy_audio_analysis("0qi4b1l0eT3jpzeNHeFXDT") %>%
  select(segments) %>%
  unnest(segments) %>%
  select(start, duration, pitches)
...
depeche %>%
  ...
  geom_tile() +
  labs(x = "Time (s)", y = NULL, fill = "Magnitude", title = "Just can't get enough - Depeche Mode") +
  theme_minimal() +
  scale_fill_viridis_c()
```
```{r}
depeche<-
  get_tidy_audio_analysis("1ChulFMnwxoD74Me8eX2TU") %>%
  select(segments) %>%
  unnest(segments) %>%
  select(start, duration, pitches)
...
depeche %>%
  ...
  geom_tile() +
  labs(x = "Time (s)", y = NULL, fill = "Magnitude", title = "You spin me round") +
  theme_minimal() +
  scale_fill_viridis_c()
```
...
The trance playlist also contains songs in C#, while this key is absent from the New Romantic playlist. The proportion of minor keys indeed seems to be slightly higher in the trance playlist.
...
The timbre plot is interesting, as it doesn't correspond to the clear checkerboard pattern shown in the chroma plot. This is quite strange, as the simple structure of the song should be reflected here too. The second chorus does feel slightly 'busier' than the first chorus, which leads me to believe there are more layers of instruments playing the same notes in the second repetition. This would explain why you couldn't detect this change in the chroma plot. At 190 seconds the timbre of the song is very different from other sections, as can be seen by the bright cross. In this section the melodies pick up in speed and become more urgent than dreamy. 
